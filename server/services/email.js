...
<head>
    ...
    <style>
        ...
        .draft-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin: 20px 0; }
    </style>
</head>
<body>
    <div class="header">
    ...
    <div class="analysis-section">
        ...
        <div class="stats">
            ...
            <div class="stat">
                ...
                <div class="stat-label">Draft Types</div>
            </div>
        </div>
    ...
    <p>Six different response approaches have been generated for your review:</p>
    ...
    <div class="draft-grid">
        ...
        <div class="draft-section">
            <h3>💰 Financial Impact Approach</h3>
            <div class="draft-content">
                ${drafts.financial ? drafts.financial.replace(/\n/g, '<br>') : 'Draft generation failed'}
            </div>
        </div>
        
        <div class="draft-section">
            <h3>🏢 Business Impact Approach</h3>
            <div class="draft-content">
                ${drafts.business ? drafts.business.replace(/\n/g, '<br>') : 'Draft generation failed'}
            </div>
        </div>
        
        <div class="draft-section">
            <h3>🌾 Livelihood Impact Approach</h3>
            <div class="draft-content">
                ${drafts.livelihood ? drafts.livelihood.replace(/\n/g, '<br>') : 'Draft generation failed'}
            </div>
        </div>
    </div>
...
${drafts.financial || 'Draft generation failed'}

BUSINESS IMPACT APPROACH:
${drafts.business || 'Draft generation failed'}

LIVELIHOOD IMPACT APPROACH:
${drafts.livelihood || 'Draft generation failed'}
...
function generateAttachments(policy, drafts) {
  const attachments = [];
  const draftTypes = ['legal', 'emotional', 'dataBacked', 'financial', 'business', 'livelihood'];
  const draftLabels = {
    legal: 'Legal',
    emotional: 'Emotional',
  ...
  draftTypes.forEach(type => {
    if (drafts[type]) {
      attachments.push({
        filename: `${policy.title.replace(/[^a-zA-Z0-9]/g, '_')}_${draftLabels[type]}_Draft.txt`,
        content: drafts[type],
  ...
  try {
    const testMailOptions = {
      ...
      html: `
        ...
        <p>The Animal Welfare Policy Monitoring System is ready to send notifications with 6 draft types:</p>
        <ul>
          <li>⚖️ Legal & Regulatory</li>
          <li>❤️ Emotional & Compassionate</li>
          <li>📊 Data-Driven & Evidence-Based</li>
          <li>💰 Financial Impact</li>
          <li>🏢 Business Impact</li>
          <li>🌾 Livelihood Impact</li>
        </ul>
        <p><small>Sent at: ${new Date().toLocaleString('en-IN')}</small></p>
      `,
      text: 'Test email successful! Your email configuration is working correctly. The system now supports 6 draft types: Legal, Emotional, Data-Driven, Financial Impact, Business Impact, and Livelihood Impact.'
    };
